...
- **Cross-pane context bleed (root cause):** the bundled v1.0 plugin shipped May 1 lacked `session_id`, `channel_label`, and `scope` on `/assemble` requests. Reinstalled the May 5 build that includes them. `:dashboard:` panes now correctly take the session-scoped retrieval path.
- **Assembler summary path:** `Message()` constructions in the oversized→summary branches at `assembler.py:202` and `:317` were dropping `channel_label` and `is_automated`. Fixed; messages now propagate identity fields end-to-end through the summary path.
