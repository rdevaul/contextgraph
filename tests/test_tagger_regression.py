"""
test_tagger_regression.py — Tagger regression test suite.
...
class TestTagRegistryCompleteness:
    ...
    def test_all_rule_tags_exist_in_yaml(self):
...
class TestNoFalsePositives:
    ...
    def test_hello(self):
        ...
        specific = {"trading", "security", "networking", "devops", "deployment"}
        overlap = tags & specific
        assert not overlap, f"Unexpected tags for greeting: {overlap}"

    def test_thanks(self):
        tags = self._tag("thanks, that's great")
        specific = {"trading", "security", "deployment"}
        overlap = tags & specific
        assert not overlap, f"Unexpected tags for thank-you: {overlap}"

    def test_generic_question(self):
        tags = self._tag("what do you think about that?")
        specific = {"security", "trading", "deployment", "devops"}
        overlap = tags & specific
        assert not overlap, f"Unexpected tags for generic question: {overlap}"
