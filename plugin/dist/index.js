 ...
 */
function migrateGraphModeIfNeeded() {
    try {
        ...
        if (typeof parsed?.enabled === "boolean") {
            const wasEnabled = parsed.enabled;
            // Migrate: preserve the global setting as __default__ so existing users keep it
            const newFormat = { enabled: {}, __default__: wasEnabled };
            ensureGraphModeDir();
...
function readGraphMode(userLabel) {
    try {
...
function writeGraphMode(userLabel, enabled) {
    ensureGraphModeDir();
    try {
        migrateGraphModeIfNeeded();
        let config = { enabled: {} };
...
// ── HTTP helper ────────────────────────────────────────────────────────────
async function apiPost(endpoint, body, logger) {
    try {
...
async function apiGet(endpoint, logger) {
    try {
        const controller = new AbortController();
        const timer = setTimeout(() => controller.abort(), REQUEST_TIMEOUT_MS);
        const response = await fetch(`${PYTHON_API_BASE}${endpoint}`, {
...
function writeComparisonLog(entry) {
    try {
        ensureGraphModeDir();
...
function countToolCalls(msg) {
    ...
    return content.filter((b) => b && typeof b === "object" &&
        (b.type === "tool_use" ||
            b.type === "toolCall")).length;
}
function safeRecentSlice(messages, maxMessages) {
...
function removeOrphanedToolPairs(messages, logger) {
    const result = [];
...
function extractTextFromMessage(msg) {
    ...
    if (typeof msg.content === "string") {
        const cleaned = stripReasoning(msg.content);
...
function splitUserAssistant(messages) {
    const userParts = [];
...
let _channelLabels = null;
let _channelLabelsSource = null;
function loadChannelLabels(logger) {
    if (_channelLabels !== null)
 ...
 */
function inferChannelLabel(senderId, sessionId, logger) {
    ...
    return {
        ...
        async bootstrap({ sessionId, sessionKey, sessionFile }) {
            ...
            try {
                ...
                messages = lines.map((l) => JSON.parse(l)).filter(Boolean);
            }
            catch (err) {
                const msg = err instanceof Error ? err.message : String(err);
        ...
        async assemble({ sessionId, sessionKey, messages, tokenBudget }) {
            ...
            if (!readGraphMode(userLabel)) {
                ...
                try {
                    ...
                    const touch = await apiPost("/current-thing/touch", {
                ...
                if (ctBlock) {
                    const withCt = [
                        { role: "user", content: [{ type: "text", text: ctBlock }] },
                        ...safe,
                    ];
            ...
            logger.info(`contextgraph assemble: ${safe.length} msgs, ~${totalTokens} tok (graph: ${data.messages?.length ?? 0} retrieved, sticky: ${stickyCount}, tools: ${lastTurnHadTools})`);
            return {
                messages: safe,
                estimatedTokens: totalTokens,
                systemPromptAddition: "Context below includes semantically relevant historical messages retrieved by the graph engine.",
            };
        },
        ...
        async afterTurn({ sessionId, sessionKey, messages, prePromptMessageCount }) {
            ...
            const hadTools = turnToolCount >= 3 ||
...
export default function register(api) {
    ...
    api.registerCommand({
        ...
        handler: async (ctx) => {
            ...
            const userLabel = inferChannelLabel(ctx.senderId, ctx.sessionId, logger);
    ...
    api.registerCommand({
        ...
        handler: async (ctx) => {
            ...
            const sessionRef = ctx.sessionKey ?? ctx.sessionId;
            if (!sessionRef) {
            ...
            if (sub === "set") {
                ...
                if (result.__error)
                    return { text: `🎯 Update failed: ${result.__error}` };
                if (result.__notFound)
                    return { text: "🎯 No snapshot exists yet for this session — send one normal message first (the snapshot is created on assemble), then retry." };
            ...
            if (sub === "clear") {
                ...
                if (result.__error)
                    return { text: `🎯 Clear failed: ${result.__error}` };
                if (result.__notFound)
                    return { text: "🎯 No snapshot exists for this session — nothing to clear." };
            ...
            if (sub === "raw") {
                ...
                if (result.__error)
                    return { text: `🎯 Fetch failed: ${result.__error}` };
                if (result.__notFound)
                    return { text: "🎯 No snapshot exists yet for this session." };
            ...
            const result = await fetchJson(`/current-thing?session_id=${sid}`);
            if (result.__error)
                return { text: `🎯 Current Thing API unreachable: ${result.__error}` };
            if (result.__notFound) {
    ...
    api.registerCommand({
        ...
        handler: async (ctx) => {
            ...
            const channelLabel = inferChannelLabel(ctx.senderId, ctx.sessionId, logger);
            ...
            if (!subcommand) {
                ...
                const { system_tags, user_tags } = tagsData;
                ...
                lines.push("");
                lines.push(`**${core.length}** system core · **${userActive.length}** user` +
                ...
                lines.push("");
                lines.push("Commands: `/tags system` · `/tags user` · `/tags user add <name>` · `/tags user del <name>`");
                return { text: lines.join("\n") };
            }
            // ── /tags system ─────────────────────────────────────────────
            ...
            if (subcommand === "user") {
                ...
                if (action === "add") {
                    ...
                    const result = await apiPost(`/tags/user/${channelLabel}/add`, { name: tagName }, logger);
                    if (!result) {
                    ...
                    const kw = r.tag?.keywords?.length
                        ? `\nKeywords: ${r.tag.keywords.map((k) => `\`${k}\``).join(", ")}`
                        : "";
                    return {
                ...
                if (action === "del" || action === "delete" || action === "rm") {
                    ...
                    const timer = setTimeout(() => controller.abort(), REQUEST_TIMEOUT_MS);
                    try {
                        const res = await fetch(`${PYTHON_API_BASE}/tags/user/${channelLabel}/${tagName}`, { method: "DELETE", signal: controller.signal });
                        clearTimeout(timer);
                        if (!res.ok) {
                ...
                lines.push("");
                lines.push("Commands: `/tags user add <name>` · `/tags user del <name>` · `/tags user retag`");
                return { text: lines.join("\n") };
            }
            return {
